...
async function fetchWithRetry(
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    ...
    if (response.status === 429 || response.status >= 500) {
      ...
      console.log(
        `Cerebras rate limit or server error (status: ${response.status}). Attempt ${attempt + 1}/${maxRetries}. Retrying in ${Math.round(delay)}ms (jitter: ${Math.round(jitter)}ms)...`,
      );
...
export class EvidenceIngestWorkflow extends WorkflowEntrypoint<
  Env,
  EvidenceIngestParams
> {
  async run(event: WorkflowEvent<EvidenceIngestParams>, step: WorkflowStep) {
    ...
    const aiResult = await step.do("analyze-evidence", async () => {
      ...
      const controlsByQs = siteControls.reduce(
        (acc, control) => {
        ...
        if (evidenceRecord.r2Key && this.env.R2) {
          ...
          if (r2Object) {
            ...
            if (
              ...
              console.log(
                `Read ${fileContentMarkdown.length} chars directly from text file.`,
              );
            }
            // Strategy 2: AI Conversion for PDFs and Images
            else if (
              this.env.AI &&
              typeof this.env.AI.toMarkdown === "function"
            ) {
              ...
              if (result && result.format === "markdown") {
                ...
                console.log(
                  `Converted ${evidenceRecord.r2Key} to ${result.tokens} tokens of markdown.`,
                );
              } else if (result?.error) {
                console.warn("Markdown conversion error:", result.error);
              } else {
                console.warn(
                  "Markdown conversion returned unexpected format:",
                  conversion,
                );
              }
            }
    ...
    await step.do("update-database", async () => {
      ...
      if (aiResult.__failed) {
        console.warn(
          `AI Analysis failed for ${evidenceId}. Marking as failed (soft-delete).`,
        );
      ...
      const safeQsId = aiResult.suggestedQsId?.includes(".")
        ? aiResult.suggestedQsId
        : "safe.safeguarding";
      ...
      const safeCategoryId = validCategories.includes(
      ...
      // Parse Date
      let evidenceDate = aiResult.evidenceDate
        ? new Date(aiResult.evidenceDate)
        : new Date();
      ...
      if (matchedControlId === "null" || matchedControlId === "undefined")
        matchedControlId = undefined;
      ...
      const classificationResult = {
        ...
        matchedControlTitle: matchedControlId
          ? (
              await db.query.localControls.findFirst({
      ...
      console.log(
        `Updated evidence ${evidenceId} with classification ${type} (${confidence}%)`,
      );
    });
  }
}
