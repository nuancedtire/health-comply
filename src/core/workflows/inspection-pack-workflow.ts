    ...
    async run(event: WorkflowEvent<InspectionPackParams>, step: WorkflowStep) {
        ...
        try {
            ...
            const aiSummaries = await step.do('generate-summaries', async () => {
                ...
                // AI is available via this.env.AI, we don't need to check API key explicitly for it
                if (!this.env.AI) {
                    console.warn("Workers AI binding not available, skipping AI summaries");
...
`;
                    ...
                    if (response && response.response) {
                        summaries.executive = response.response;
                    } else {
                        console.warn("AI response returned empty content");
                    }
                } catch (error) {
...
Provide a compliance assessment for this domain.`;
                        ...
                        if (response && response.response) {
                            summaries[kq.id] = response.response;
                        } else {
                            console.error(`KQ summary failed for ${kq.id}: Empty response`);
                        }
                    } catch (error) {
...
`);
            ...
            const pdfR2Key = await step.do('build-pdf', async () => {
                ...
                try {
                    ...
                    console.log(`PDF buffer generated, size: ${pdfBuffer.byteLength} bytes`);

                    // Upload to R2
                    const r2Key = `t/${tenantId}/packs/${packId}/report.pdf`;
                    console.log(`Uploading PDF to R2: ${r2Key}`);
                    ...
                    console.error("Error in build-pdf step:", err);
                    throw err;
                }
            });
            ...
            console.error(`Workflow failed for pack ${event.payload.packId}:`, error);
            try {
                const db = drizzle(this.env.DB, { schema });
                await db.update(schema.inspectionPacks)
                    .set({ status: 'error' })
                    .where(eq(schema.inspectionPacks.id, event.payload.packId));
            } catch (dbError) {
                console.error("Failed to update pack status to error:", dbError);
            }
